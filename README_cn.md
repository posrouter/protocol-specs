...
| **V1.4** | 联盟 Lensing 整合规范 | 三层集成路线图；JSON 载荷；NATS Subject；Gateway `/init` HMAC；全景与路由 Mermaid 图 |
| **V1.5** | **当前** | Level 1 新增 **`void`**、回调 **`card_number`**（卡号末四位）；规范拆分为三层独立文件；中英双语 |

> **命名说明：** 旧 README 中的「V0.4」对应规范 **V1.4**；含 void 与文档重组后为 **V1.5**。
...
| **2** | [level-2-lensing_cn.md](./level-2-lensing_cn.md) | [level-2-lensing_en.md](./level-2-lensing_en.md) | Gateway `/init` + NATS（`.pay` / `.result` / `.claimed` / **`.void`**）+ JSON | 可选 |
| **3** | [level-3-signed_cn.md](./level-3-signed_cn.md) | [level-3-signed_en.md](./level-3-signed_en.md) | Level 2 + 客户端非对称密钥、签名信封、Participant 证书 | 强烈建议 |
...
graph TD
    ...
    subgraph Scenario_2 ["场景二：跨设备集群 (Level 2+)"]
        B -->|"网络轨道: NATS 广播"| F["Lensing 分布式虚拟网络"]
        F -->|"lensing.terminal.TID.pay"| G["边缘终端 A<br>(远程商米 P3)"]
...
graph TD
    ...
    NetTrack --> Envelope["JSON 载荷 · NATS 广播"]
...
| 生产联盟、非对称鉴权 | [Level 3 中文](./level-3-signed_cn.md) |
